...
export default function App() {
  ...
  // Save score when game ends and user is logged in
  useEffect(() => {
    if (gameEnded && username && !scoreSaved.current) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center justify-center p-8">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-3">
        {username ? (
          <>
            <span className="text-amber-900">Welcome, {username}</span>
            <ScoreHistory />
            <Button variant="outline" className="border-amber-400 text-amber-900 hover:bg-amber-200" onClick={logout}>
              Logout
            </Button>
          </>
        ) : (
